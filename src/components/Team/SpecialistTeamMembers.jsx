        ...
        <div className="sticky-content container">
          <div className="content style_2">
            <div className="service">
              {speciaList.map(item => (
                <div className="service-card" data-aos="fade-left" key={item?.id}>
                  <Link to={`/service-single/${item?.id}`} className="card-img">
                    <img src={`${item?.img}`} className="ak-bg" alt="..." />
                  </Link>
                  <div className="card-info">
                    <Link to={`/service-single/${item?.id}`} className="card-title">
                      {item?.title}
                    </Link>
                    <p className="card-desp">{item?.desp}</p>
                    <MoreBtn to={`/service-single/${item?.id}`}>VIEW MORE</MoreBtn>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="sidebar width-none">
            <div className="services-content">
              <SectionHeading bgText={'Services'} title={team?.name} desp={team?.desp} />
            </div>
          </div>
        </div>
      </div>
